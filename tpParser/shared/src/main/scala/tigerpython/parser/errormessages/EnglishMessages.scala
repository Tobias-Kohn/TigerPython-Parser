...
object EnglishMessages extends ModifiableErrorMessages {
  ...
  protected def _getMessage(msg: ErrorCode.Value): String =
    msg match {
      case AND_CONNECTS_CMP_NOT_VALUES => "'%s' connects comparisons not values."
      ...
      case CANNOT_USE_STAR_IN_DICT => "Cannot use a starred expression in a dictionary value."
      case CASE_REQUIRED => "A 'match'-block can only contain 'case'-statements."
      ...
      case DOUBLE_CALL => "You are calling the turn argument of a function here."
      ...
      case DOUBLE_WALRUS => "You need to use parenthesis to assign an expression to two names: '%s := (%s := ...)'."
      ...
      case EXTRA_BRACKETS => "The keyword '%s' does not have parentheses '()'."
      case EXTRA_DEF => "There is an extra 'def' here."
      ...
      case EXTRA_LINE_NUMBER => "There seem to be line number at the start of the line, which should be removed."
      ...
      case INVALID_FUNCTION_DEF_ASSIGN => "Use ':' and 'return' or 'lambda' instead of an assignment."
      case INVALID_GENERATOR_ARG => "Generator expression must be parenthesised."
      ...
      case MISSPELLED_NAME => "Misspelled name '%s' instead of '%s'."
      ...
      case USE_UNDERLINE_NOT_MINUS => "Use an underline '_' here, not a hyphen."
